...
// VAPID key voor web push — haal op uit Firebase Console
// Build → Cloud Messaging → Web Push certificates → Key pair
const VAPID_KEY = 'JOUW_VAPID_KEY';
...
async function ensureHousehold(user) {
  ...
  if (snap.exists() && snap.data().householdId) {
    ...
    partnerPhone = snap.data().partnerPhone || null;
...
async function setupPushNotifications() {
  try {
    ...
    if (permission !== 'granted') {
      console.log('Push notificaties geweigerd');
      return;
    }

    // Registreer service worker
    const registration = await navigator.serviceWorker.register('/Shopping/firebase-messaging-sw.js');
    ...
    if (token) {
      ...
      // Voeg huidige gebruiker toe aan household members als dat nog niet zo is
      const hSnap = await getDoc(doc(db, 'households', householdId));
      if (hSnap.exists()) {
    ...
    // Verwerk notificaties als de app op de voorgrond is
    onMessage(messaging, payload => {
      const { title, body } = payload.notification;
      // Toon de banner in de app
      document.getElementById('going-who').textContent = body;
      document.getElementById('going-banner').classList.remove('hidden');
      setTimeout(() => {
        document.getElementById('going-banner').classList.add('hidden');
      }, 10 * 60 * 1000);
    });

  } catch (err) {
    console.log('Push setup mislukt (normaal in development):', err.message);
  }
}
...
window.savePartnerPhone = async function() {
  const input = document.getElementById('settings-partner-phone').value.trim();
  // Verwijder spaties, +, en streepjes
  const cleaned = input.replace(/[\s+\-()]/g, '');
  if (!cleaned) return;
  partnerPhone = cleaned;
  await updateDoc(doc(db, 'users', currentUser.uid), { partnerPhone: cleaned });
...
function initApp() {
  ...
  document.getElementById('settings-partner-phone').value    = partnerPhone || '';
  ...
  // Stel push notificaties in na korte vertraging zodat de app eerst laadt
      ...
      <div class="fav-card">
        <div class="fav-header" onclick="toggleFav('${f.id}')">
          ...
          <div class="fav-header-right">
            ...
            <button class="btn-fav-edit" onclick="event.stopPropagation(); openEditFav('${f.id}')">✏️</button>
...
window.toggleFav     = function(id) { favOpenState[id] = !favOpenState[id]; renderFavs(); };
...
window.openEditFav = function(id) {
  ...
  editFavId = id;
  document.getElementById('modal-edit-fav-name').value = f.name;
  selectedEmoji = f.emoji || '🛒';
  document.getElementById('edit-emoji-grid').innerHTML = EMOJIS.map(e =>
    `<div class="emoji-opt ${e === selectedEmoji ? 'selected' : ''}" onclick="selectEditEmoji(this,'${e}')">${e}</div>`
  ).join('');
  openModal('modal-edit-fav');
};

window.selectEditEmoji = function(el, emoji) {
  document.querySelectorAll('#edit-emoji-grid .emoji-opt').forEach(e => e.classList.remove('selected'));
  el.classList.add('selected');
  selectedEmoji = emoji;
};

window.saveEditFav = async function() {
  const name = document.getElementById('modal-edit-fav-name').value.trim();
  if (!name || !editFavId) return;
  await updateDoc(doc(db, 'households', householdId, 'favorites', editFavId), { name, emoji: selectedEmoji });
  closeModal('modal-edit-fav');
};

window.deleteFavList = async function() {
  if (!editFavId) return;
  if (!confirm('Weet je zeker dat je deze lijst wilt verwijderen?')) return;
  await deleteDoc(doc(db, 'households', householdId, 'favorites', editFavId));
  closeModal('modal-edit-fav');
  editFavId = null;
};
...
window.selectEmoji = function(el, emoji) {
  document.querySelectorAll('#emoji-grid .emoji-opt').forEach(e => e.classList.remove('selected'));
...
window.createFavList = async function() {
  ...
  const newRef = await addDoc(collection(db, 'households', householdId, 'favorites'), {
  ...
  if (activeItemId && items[activeItemId]) {
    await updateDoc(doc(db, 'households', householdId, 'favorites', newRef.id), {
      items: [items[activeItemId].name]
    });
    activeItemId = null;
  }
};
...
function renderDeals() {
  const filtered = DEALS.filter(d => {
    if (currentDealFilter === 'match') return Object.values(items).some(i => i.cat === d.catMatch);
...
window.startGoing = async function() {
  ...
  // Stuur WhatsApp bericht naar partner
  if (partnerPhone) {
    const msg = encodeURIComponent('🛒 Ik ga zo boodschappen doen! Voeg snel nog iets toe aan de lijst: https://dwayneschreiner.github.io/Shopping/');
    window.open('https://wa.me/' + partnerPhone + '?text=' + msg, '_blank');
  } else {
    alert('Vul eerst het WhatsApp-nummer van je partner in via ⚙️ Instellingen.');
    return;
  }

  // Sla op in Firestore zodat partner banner ziet als app open is
  await setDoc(doc(db, 'households', householdId, 'notifications', 'going'), {
    who: currentUser.email.split('@')[0], startedAt: serverTimestamp()
  });

  // Toon banner in eigen app
  ...
  goingInterval = setInterval(() => {
    ...
    if (secs <= 0) { clearInterval(goingInterval); document.getElementById('going-banner').classList.add('hidden'); }
  }, 1000);
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('modal-add-input')?.addEventListener('keydown', e => { if (e.key==='Enter') window.addFromModal(); });
  document.getElementById('ai-input')?.addEventListener('keydown', e => { if (e.key==='Enter') window.handleAIInput(); });
});
