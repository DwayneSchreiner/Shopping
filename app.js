      ...
      <div class="fav-card">
        <div class="fav-header" onclick="toggleFav('${f.id}')">
          ...
          <div class="fav-header-right">
            ...
            <button class="btn-fav-edit" onclick="event.stopPropagation(); openEditFav('${f.id}')" title="Bewerken">✏️</button>
...
window.openEditFav = function(id) {
  ...
  // Zet emoji selector
  const currentEmoji = f.emoji || '🛒';
  selectedEmoji = currentEmoji;
  document.getElementById('edit-emoji-grid').innerHTML = ['🥛','🥩','🥦','💪','🍳','🛒','🥗','🍺','🧃','🍰','🐟','🌿','🧴','🏠'].map((e,i) =>
    `<div class="emoji-opt ${e===currentEmoji?'selected':''}" onclick="selectEditEmoji(this,'${e}')">${e}</div>`
  ).join('');
...
window.saveEditFav = async function() {
  ...
  await updateDoc(doc(db, 'households', householdId, 'favorites', editFavId), {
...
window.openEditFav = function(id) {
  ...
  document.getElementById('edit-emoji-grid').innerHTML = EDIT_EMOJIS.map(e =>
